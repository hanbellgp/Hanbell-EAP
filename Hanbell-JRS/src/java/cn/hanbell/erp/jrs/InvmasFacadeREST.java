...
public class InvmasFacadeREST extends SuperRESTForERP<Invmas> {
    ...
    @Override
    public List<Invmas> findByFilters(@PathParam("filters") PathSegment filters, @PathParam("sorts") PathSegment sorts, @PathParam("offset") Integer offset, @PathParam("pageSize") Integer pageSize, @QueryParam("appid") String appid, @QueryParam("token") String token) {
        if (isAuthorized(appid, token)) {
            List<Invmas> invmasList;
            try {
                ...
                invmasList = invmasBean.findByFilters(filterFields, offset, pageSize, sortFields);
                return invmasList;
            } catch (Exception ex) {
