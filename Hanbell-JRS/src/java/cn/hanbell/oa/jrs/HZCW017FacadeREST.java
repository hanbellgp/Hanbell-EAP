...
public class HZCW017FacadeREST extends SuperREST<HZCW017> {
    ...
    public ResponseMessage create(MCHZCW017 entity) {
        if (entity == null) {
        ...
        try {
            ...
            la.setLoanDept(workFlowBean.getUserFunction().getOrganizationUnit().getId());
            la.setHdnloanDept(workFlowBean.getUserFunction().getOrganizationUnit().getOrganizationUnitName());
            //根据部门编号代出公司编号
            la.setLoanProperty(entity.getLoanproperty().split("-")[0]);
            la.setHdnloanProperty(entity.getLoanproperty().split("-")[1]);
            la.setCreator(entity.getLoanUser());
            la.setAmount("");              //大写金额
            la.setReason(entity.getReason());
            la.setCoin(entity.getCoin());
            misrateBean.setCompany(entity.getCompany());
            ...
            la.setPreCustomer("");
            budgetDetailBean.setCompany(entity.getCompany());
            la.setDeptPeriod(budgetDetailBean.getBudgetBalanceForDeptPeriod(entity.getCompany(), BaseLib.getDate(), entity.getCenterid()));
            la.setDeptYear(budgetDetailBean.getBudgetBalanceForDeptYear(entity.getCompany(), BaseLib.getDate(), entity.getCenterid()));
