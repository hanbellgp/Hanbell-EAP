...
public class TaskFacadeREST extends SuperRESTForEAP<Task> {
    ...
    public ResponseData findByExecutorId(@PathParam("executorid") String executorid, @QueryParam("status") String status,
    ...
    @Produces({MediaType.APPLICATION_JSON})
    public ResponseObject findByManagerIdAndProgress(@PathParam("managerid") String managerid, @QueryParam("appid") String appid, @QueryParam("token") String token) {
        if (isAuthorized(appid, token)) {
            Map<String, Integer> obj = new HashMap<>();
            Calendar c = Calendar.getInstance();
            Integer thisYearFinished, thisMonthFinished, unfinished;
            try {
                ...
                thisMonthFinished = taskBean.getRowCountByManagerIdYearAndMonth(managerid, c.get(Calendar.YEAR), c.get(Calendar.MONTH) + 1);
    ...
    @Produces({MediaType.APPLICATION_JSON})
    public ResponseData findByManagerIdAndPastWeeks(@PathParam("managerid") String managerid, @PathParam("weeks") Integer weeks, @QueryParam("offset") Integer offset, @QueryParam("pageSize") Integer pageSize,
            @QueryParam("appid") String appid, @QueryParam("token") String token) {
        if (isAuthorized(appid, token)) {
            try {
                Calendar c = Calendar.getInstance();
    ...
    @Produces({MediaType.APPLICATION_JSON})
    public ResponseData findByManagerIdAndNextWeeks(@PathParam("managerid") String managerid, @PathParam("weeks") Integer weeks, @QueryParam("offset") Integer offset, @QueryParam("pageSize") Integer pageSize,
            @QueryParam("appid") String appid, @QueryParam("token") String token) {
        if (isAuthorized(appid, token)) {
            try {
                Calendar c = Calendar.getInstance();
