...
public class InvclsBean extends SuperEJBForERP<Invcls> {
    ...
    public Boolean createByOAHKJS007(String psn) {
        ...
        try {
            ...
            String[] gcheck = {h.getComputeGcheck1(), h.getComputeGcheck2(), h.getComputeGcheck3(), h.getComputeGcheck4(), h.getComputeGcheck5(), h.getComputeGcheck6(), "N", "N", "N", "N", "N", "N"};
            for (int n = 1; n < gcheck.length; n++) {
                if (gcheck[n].equals("0")) {
                    gcheck[n] = "N";
                }
            }
            String[] gitnbr = {h.getComputeGitnbr1(), h.getComputeGitnbr2(), h.getComputeGitnbr3(), h.getComputeGitnbr4(), h.getComputeGitnbr5(), h.getComputeGitnbr6(), "N", "N", "N", "N", "N", "N"};
            for (int i = 0; i < gitnbr.length; i++) {
                if (gitnbr[i].equals("0")) {
                    gitnbr[i] = "N";
                }
            }
            String[] gitdsc = {h.getComGitdsc1(), h.getComGitdsc2(), h.getComGitdsc3(), h.getComGitdsc4(), h.getComGitdsc5(), h.getComGitdsc6(), "N", "N", "N", "N", "N", "N"};
            for (int j = 0; j < gitdsc.length; j++) {
                if (gitdsc[j].equals("0")) {
                    gitdsc[j] = "N";
                }
            }
            String[] gspdsc = {h.getComGspdsc1(), h.getComGspdsc2(), h.getComGspdsc3(), h.getComGspdsc4(), h.getComGspdsc5(), h.getComGspdsc6(), "N", "N", "N", "N", "N", "N"};
            for (int k = 0; k < gspdsc.length; k++) {
                if (gspdsc[k].equals("0")) {
                    gspdsc[k] = "N";
                }
            }
            String[] gmodel = {h.getComGmodel1(), h.getComGmodel2(), h.getComGmodel4(), h.getComGmodel4(), h.getComGmodel5(), h.getComGmodel6(), "N", "N", "N", "N", "N", "N"};
