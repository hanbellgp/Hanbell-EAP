...
@NamedQueries({
    ...
    @NamedQuery(name = "Purhad.findNeedThrowByVdrnoAndHmark1", query = "SELECT p FROM Purhad p WHERE p.vdrno = :vdrno AND p.podate>= :podate AND p.hmark1 = :hmark1 AND p.hposta='Y' AND (p.fromcdrno IS NULL OR p.fromcdrno='') ORDER BY p.purhadPK.pono")})
