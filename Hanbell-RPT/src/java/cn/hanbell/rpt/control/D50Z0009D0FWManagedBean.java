    ...
    public void update() {
        try {
            ...
            for (Object[] o : selectData) {
                ...
                d.setTravelAmount((Double) o[1]);
                d.setHours((Double) o[2]);
                d.setMaterialAmount((Double) o[3]);
                detailList.add(d);
            }
            details.put("detail", detailList);
            String formInstance = workFlowBean.buildXmlForEFGP("D50_Z0009_D0_FW", head, details);
            String msg = workFlowBean.invokeProcess(workFlowBean.HOST_ADD, workFlowBean.HOST_PORT, "PKG_HK_GL034", formInstance, applyUser.getUsername() + "集团服务工作支援单");
            String[] rm = msg.split("\\$");
            if (rm.length == 2) {
